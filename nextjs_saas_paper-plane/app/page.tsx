...
import { resendEmailConfigured, resendUsingTestingAddress } from '@/lib/resend-config';
          ...
          <ul className="relative grid w-full grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
            ...
            <li className="relative">
              {resendEmailConfigured ? (
                ...
                >
                  <div className="relative">
                    <img alt="" className="inline-block h-8 w-8 rounded-sm" src="/icons/resend.svg" />
                    <div className="mb-1.5 mt-3 flex items-center gap-2">
                      <h3 className="text-lg font-medium text-neutral-950">Resend</h3>
                      <StatusBadge label={resendUsingTestingAddress ? 'Testing address' : 'Configured'} tone={resendUsingTestingAddress ? 'pending' : 'configured'} />
                    </div>
                    <p className="mb-3 text-pretty text-neutral-600">
                      {resendUsingTestingAddress
                        ? 'Resend sends the welcome email from onboarding@resend.dev, which only delivers to the email on your Resend account. Verify a domain and set RESEND_FROM_ADDRESS to reach subscribers.'
                        : 'Resend sends the welcome email that greets subscribers the moment their subscription goes active.'}
                    </p>
                  </div>
                  <div className="flex items-center gap-1 font-medium">
                ...
                <div className="relative flex h-full flex-col rounded-md border border-neutral-950/10 p-6">
                  <div className="relative">
                    ...
                    <p className="mb-3 text-pretty text-neutral-600">
                      Resend sends the welcome email when a subscription starts. Finish setup to turn it on:
                    </p>
                  </div>
                  <ol className="flex flex-col gap-2 text-sm text-neutral-600">
                    <li className="flex gap-2">
                      ...
                      <span>
                        Run{' '}
                        <code className="rounded bg-neutral-950/5 px-1 py-0.5 font-mono text-xs text-neutral-950">stripe projects add resend/email</code>
                        {' '}and then{' '}
                        <code className="rounded bg-neutral-950/5 px-1 py-0.5 font-mono text-xs text-neutral-950">stripe projects env --pull</code>
                        {' '}to write{' '}
                        <code className="rounded bg-neutral-950/5 px-1 py-0.5 font-mono text-xs text-neutral-950">RESEND_API_KEY</code>.
                      </span>
                    </li>
                    <li className="flex gap-2">
                      <span className="font-mono text-xs text-neutral-400">2.</span>
                      <span>
                        Verify a domain in the Resend dashboard and set{' '}
                        <code className="rounded bg-neutral-950/5 px-1 py-0.5 font-mono text-xs text-neutral-950">RESEND_FROM_ADDRESS</code>
                        {' '}(and optionally{' '}
                        <code className="rounded bg-neutral-950/5 px-1 py-0.5 font-mono text-xs text-neutral-950">RESEND_FROM_NAME</code>).
                      </span>
                    </li>
                  </ol>
                </div>
