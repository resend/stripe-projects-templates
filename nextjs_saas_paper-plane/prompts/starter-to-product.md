...
1. Starter audit
   ...
   - Also inspect `app/api/webhooks/stripe/route.ts`, `lib/resend-config.ts`, and `lib/resend-email.ts` to see the Resend transactional email integration wired to the subscription webhook.
...
4. Phased implementation
   ...
   - Resend transactional email is already wired in. When a subscription starts, a best-effort "welcome / access granted" email goes out through `lib/resend-email.ts`, authenticated with the provisioned `RESEND_API_KEY`. Treat this as a real product surface, not scaffolding:
     - Both the Stripe webhook (`app/api/webhooks/stripe/route.ts`) and the success page (`app/success/page.tsx`) trigger the send, so a subscriber is emailed whether or not they return to the site. They call the shared `sendWelcomeEmailForSubscriptionOnce` helper, which claims an atomic guard in the database (`welcome_email_sent_at` on `subscriptions`) so exactly one email goes out per subscription even if both paths fire at once.
     - Rewrite the email subject and body in `lib/resend-email.ts` to match the product name, voice, and offer instead of the generic starter copy. React Email (`@react-email/components`) is a good fit if the product needs richer templates.
     - Consider it for other lifecycle moments the product needs: subscription receipts, onboarding sequences, plan changes and renewal notices, and account or magic-link style notifications.
     - Keep sends best-effort so an email failure never blocks checkout or the webhook. The default from address `onboarding@resend.dev` delivers only to the email on the Resend account; a verified domain and `RESEND_FROM_ADDRESS` are needed to reach subscribers.
     - The home page shows a Resend status card (`app/page.tsx`) that flips to setup instructions when `RESEND_API_KEY` is unset and flags the testing address until `RESEND_FROM_ADDRESS` is set. Keep it in sync if you rename or repurpose the integration.

5. Verification
   - Verify `/api/health` still reflects the app correctly, including `resendEmailConfigured`.
   - Verify the main user flow still works for the integrations that are present.
   - If Resend email copy changed, verify a completed test subscription still sends the welcome email and the webhook still returns 200.
